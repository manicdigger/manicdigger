public class ModListWidget : AbstractMenuWidget
{
    ModButtonWidget[] listButtons;
	...
	public override void Draw(float dt, UiRenderer renderer)
	{
		...
		if (lastSizeY != sizey)
		{
			...
			listButtons = new ModButtonWidget[entriesPerPage];
			for (int i = 0; i < entriesPerPage; i++)
			{
                ModButtonWidget b = new ModButtonWidget();
				b.SetVisible(false);
				listButtons[i] = b;
			}
			UpdateScrollButtons();
...
public class ModButtonWidget : ServerButtonWidget {
    bool active;
    public override void OnMouseDown(GamePlatform p, MouseEventArgs args)
    {
        bool temp = hasKeyboardFocus;
       // base.OnMouseDown(p, args);
        if(temp && hasKeyboardFocus)
        {
            active = !active;
