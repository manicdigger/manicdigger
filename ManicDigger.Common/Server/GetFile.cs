	...
	public class GetFileStream : IGetFileStream
	{
		...
		public Stream GetFile(string filename)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				if (cache.ContainsKey(filename))
				{
					return new MemoryStream(cache[filename]);
				}

				string f1 = filename.Replace(".png", ".jpg");
				if (cache.ContainsKey(f1))
				{
					remap[origfilename] = f1;
					goto retry;
				}
				...
				{
					remap[origfilename] = f2;
					goto retry;
				}

				string f3 = filename.Replace(".wav", ".ogg");
				if (cache.ContainsKey(f3))
				{
					remap[origfilename] = f3;
					goto retry;
				}

				string f4 = filename.Replace(".ogg", ".wav");
				if (cache.ContainsKey(f4))
				{
					remap[origfilename] = f4;
					goto retry;
				}
